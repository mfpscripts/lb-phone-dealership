...
window.addEventListener("load", () => {
    $.post('https://lb-phone-dealership/getVehicles', JSON.stringify({}), function(data) {
        ...
        if(newInner == '') {
            for(let i in vehicles) {
                for(let j in categories) {
                    if(categories[j].id == vehicles[i].category) {
                        let inner = `
                            <div id="vehicle-card-${vehicles[i].model}-${vehicles[i].brand}" class="vehicle-card"> 
                                <img src="${vehicles[i].pic}" onerror="this.src='assets/alt.png'"> 
                                <div class="vehicleInfo"> 
                                    <div class="vehicle-label">${vehicles[i].name} - ${vehicles[i].brand}</div> 
            ...
            newInner = `<div id="all" class="category">Alle</div>`
